...
Public Class ModelCodeGenerator
    ...
    Private Sub GenerateModelCode(ByVal provider As CodeDomProvider, ByVal codeGenOptions As CodeGeneratorOptions)
        ...
        m_modelProjects(CodeProjectFile.PROJECTNAME_QUERY_DEFINITION).AddReference(CodeProjectFile.REFERENCE_IDENTITYGROUP)
        ...
        m_modelProjects(CodeProjectFile.PROJECTNAME_COMMAND_DEFINITION).AddReference(CodeProjectFile.REFERENCE_IDENTITYGROUP)
        ...
        For Each aggregate As AggregateIdentifier In m_model.AggregateIdentifiers
            ...
            For Each eventDef As EventDefinition In aggregate.EventDefinitions
                ...
                CreateSourceCodeFile(MakeInterfaceName(eventCodeGen.FilenameBase),
                                     eventCodeGen.GenerateInterface(), m_options, m_model.Name, ModelSourceFileType.EventDefinitionInterface)
                'Make the class for the event definition
                CreateSourceCodeFile(MakeImplementationClassName(eventCodeGen.FilenameBase),
                                     eventCodeGen.GenerateImplementation(), m_options, m_model.Name, ModelSourceFileType.EventDefinition)
                ...
                eventSerialisationCodeGen.SetCodeGenerationOptions(m_options)
                CreateSourceCodeFile(MakeImplementationClassName(eventSerialisationCodeGen.FilenameBase),
                                     eventSerialisationCodeGen.GenerateImplementation(), m_options, m_model.Name, ModelSourceFileType.EventDefinition)

                'and the fast-mode (non reflection) serialiser factory
                Dim eventSerialisationFactoryCodeGen As New EventSerializerCodeGenerator(eventDef)
                eventSerialisationFactoryCodeGen.SetCodeGenerationOptions(m_options)
                CreateSourceCodeFile(MakeImplementationClassName(eventSerialisationFactoryCodeGen.FilenameBase),
                                     eventSerialisationFactoryCodeGen.GenerateImplementation(), m_options, m_model.Name, ModelSourceFileType.EventDefinition)
...
#End Region
    ...
    Public Sub CreateSourceCodeFile(ByVal filenameBase As String,
        ...
        Dim filenameReference As String = filenameBase
        If (options.SeparateFolderPerModel) Then
            If Not String.IsNullOrWhiteSpace(modelName) Then
                filenameReference = System.IO.Path.Combine(MakeValidCodeName(modelName), filenameBase)
            End If
        End If
        AddSourceFileToProjects(filenameReference, sourceFileType)
    ...
    Public Shared Function ParameterisedConstructor(eventProperties As IList(Of IEventPropertyEntity)) As CodeConstructor
        ...
        If (eventProperties IsNot Nothing) Then
            For Each eventProperty As IEventPropertyEntity In eventProperties
                fromParametersConstructor.Parameters.Add(New CodeParameterDeclarationExpression(EventCodeGenerator.ToMemberType(eventProperty.DataType),
                                                                                                EventCodeGenerator.ToMemberName(eventProperty.Name, False, True)))
...
Public Class MethodCodeGenerator
    ...
    Public Shared Function PublicParameterisedFunction(ByVal functionName As String,
                                                       ByVal returnType As CodeTypeReference,
                                                       ByVal parameters As IList(Of CodeParameterDeclarationExpression)) As CodeMemberMethod
    ...
    Public Shared Function PublicParameterisedFunction(ByVal functionName As String,
                                              ...
                                              Optional ByVal makeStatic As Boolean = False,
                                              Optional ByVal makeOverride As Boolean = False,
                                              Optional ByVal genericTypeRestrictions As CodeTypeParameterCollection = Nothing) As CodeMemberMethod

        Dim ret As New CodeMemberMethod()
        ret.Name = functionName
        If (makeStatic) Then
            'Make this static and public
            ret.Attributes = (ret.Attributes And (Not MemberAttributes.AccessMask)) Or (MemberAttributes.Static Or MemberAttributes.Public)
        Else
            ret.Attributes = (ret.Attributes And (Not MemberAttributes.AccessMask)) Or MemberAttributes.Public
        End If
        If makeOverride Then
            ret.Attributes += MemberAttributes.Override
        End If
    ...
    Public Shared Function PrivateParameterisedFunction(ByVal functionName As String,
        ...
        If (makeStatic) Then
            ret.Attributes = (ret.Attributes And (Not MemberAttributes.AccessMask)) Or (MemberAttributes.Static Or MemberAttributes.Private)
        Else
...
#Region "Defined attributes"
    ...
    Public Const ATTRIBUTENAME_AGGREGATE_KEY As String = "CQRSAzure.EventSourcing.AggregateKey"
